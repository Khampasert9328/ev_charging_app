  ...
  static Future updateStationService(context, String id, DataGetInfoCharg? data) async {
    ...
    for (int i = 0; i < model.containersList.length; i++) {
      oldContainer.add({
        ...
        "type_charge": [
          for (var j in model.containersList[i].typeChargingList) {"type_charging": j.index == i ? j.typeCharging : ""}
        ],
      });
    }
    ...
    final res = await http.put(
      Uri.parse(AppDomain.updateStation + id),
      headers: {
        "accept": "text/plain",
    ...
    if (res.statusCode == 200) {
      ...
      showDialog(
          context: context,
          builder: (_) => DialogSucces(
                onTap: () async{
                  model.clearForm();
                 await Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(
                      builder: (context) => const ManagemaentStation(),
                    ),
                  );
                },
